    ...
    <div className="admin-users">
      {users.map((user) => {
        ...
        return <article className="panel admin-user" key={user.id}>
          <div><strong>{user.login}</strong><small>Зарегистрирован: {new Date(user.createdAt).toLocaleDateString('ru-RU')}</small><small>{user.role === 'admin' ? 'Безлимитные попытки' : `Доступно попыток: ${user.attemptsRemaining}`}</small></div>
          ...
          {user.role === 'user' && <button className="primary-button" type="button" onClick={() => action(() => grantTestAttempt(user.id))}>Открыть пересдачу</button>}
